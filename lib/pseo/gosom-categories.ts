...
/**
 * Maps the category label gosom reads off Google Maps onto the place type `leads.category` holds.
 *
 * These are two different vocabularies. The Places API returns a `primaryType` from a closed list
 * of a few hundred; Maps shows a far more granular human label, and India-specific ones at that —
 * "Beauty Parlour", "Coaching Center", "Chicken & Mutton Shop". Snake-casing the label matches
 * about half the time and silently drops the rest, which on the first real run meant 1,794 of 3,499
 * scraped businesses were thrown away.
 *
 * Anything not listed here and not already a valid place type stays dropped, on purpose. That
 ...
 */
export function gosomCategoryToPlaceType(...labels: Array<string | undefined | null>): string | null {
  for (const label of labels) {
    ...
    if (alias && !TYPE_TO_SECTION[alias]) {
      console.warn(`gosom-categories: alias "${key}" -> "${alias}" is not a known place type`);
    }
  }
  return null;
}
